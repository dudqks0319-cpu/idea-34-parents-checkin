...
class SupabaseService {
  ...
  Future<void> saveTask({required String userId, required EnergyTask task}) async {
    if (useSupabase) {
      await Supabase.instance.client.from('parents_checkin_items').upsert(task.toInsertMap(userId), onConflict: 'id');
      return;
    }
  ...
  Future<void> deleteTask({required String userId, required String taskId}) async {
    if (useSupabase) {
      await Supabase.instance.client.from('parents_checkin_items').delete().eq('id', taskId).eq('user_id', userId);
      return;
    }
